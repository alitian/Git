...
<div>
    <ul class="breadcrumb">
        ...
        <li>
            <a href="history_data">历史数据</a>
        </li>
    </ul>
</div>
<div class="row-fluid sortable">
    <div class="box span12">
        <div id="p" class="easyui-panel" title="历史数据-查询条件" style="height:280px;padding:10px;margin: 0;" data-options="collapsible:true">
                ...
                <div class="span10">
                    <div class="span3">展示选择<br />
                        <select id="show_status" name="show_status" onchange='show_select();'>
                            ...
                            <option value="4">按日期（同一时间内,ad参数的整体数据）</option>
                        </select>
                    </div>
                </div> 
                    ...
                    <div class="span10">                       
                        <div class="row-fluid"> 
                            ...
                            <div id="for_date" style="display:none">
                                <div class="span4">ad_id :<br />
                                    <input type="text" id='ad_id4' name='ad_id4' value="" placeholder="多个请用,分割" />
                                </div> 
                                <div class="span4">provider_id :<br />
                                    <input type="text" id='provider_id4' name='provider_id4' value="" placeholder="多个请用,分割" />
                                </div>
                            </div>                         
                        </div>
                        <div class="row-fluid">                          
                            <div class="span4">筛选时间（start）：<br>
                                <div id="datetimepicker1" class="input-append">
                                    <input data-format="yyyy-MM-dd" type="text" placeholder="筛选时间（start）" name='start_time' id='start_time' value="<?= $startTime ?>"></input>
                                    <span class="add-on">
                            ...
                            <div class="span4">筛选时间（end）：<br>
                                <div id="datetimepicker2" class="input-append">
                                    <input data-format="yyyy-MM-dd" type="text" placeholder="筛选时间（end）" name='end_time' id='end_time' value="<?= $startTime ?>"></input>
                                    <span class="add-on">
                                        <i data-time-icon="icon-time" data-date-icon="icon-calendar">
                                        </i>
                                    </span>
        ...
        <div id="p2" class="easyui-panel" title="历史数据-数据图" style="height:auto;padding:10px;margin: 0;" data-options="collapsible:true">
        ...
        <div class="box-content" style="overflow:scroll;height:500px;">
            <table id="tt" class="easyui-datagrid" style="margin: 0;" title="查询结果" 
                   url="/statistics/ajax_history_data" title="Load Data" iconCls="icon-save"
...
$(document).ready(function() {
    $('#btn_search').click(function(e) {
        ...
        if (show_status == 4) {
            select_obj.type = '';
            select_obj.provider_id = $('#provider_id4').val();
            select_obj.ad_id = $('#ad_id4').val();
            select_obj.ad_name ='';
        }
        select_obj.status = $('#status').val();
        select_obj.start_time = $('#start_time').val();
        select_obj.end_time = $('#end_time').val();
        select_obj.bysort = $('#bysort').val();
        select_obj.byorder = $('#byorder').val();
        e.preventDefault();
        form1.action = "/statistics/ajax_history_data";
        $('#tt').datagrid('load', {
    ...
    $("#tt").datagrid({
        onLoadSuccess: function(data) {
            ...
            if (show_status == 4) {
                ...
                var end_time = $("#end_time").val();
                options.chart.type = 'spline';
                options.title.text = start_time + '~' + end_time + '下发/安装/回调/数据走势';
                options.subtitle.text = 'source:ad_name间对比';
                for (var i = 0; i < (obj.length); i++) {
                    categories.push(obj[i].ad_name+'('+obj[i].date+')');
                }
            }
