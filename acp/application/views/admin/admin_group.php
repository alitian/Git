    ...
    <div class="easyui-layout" data-options="fit:true">
        <div data-options="region:'west',split:true" style="width:500px;padding:10px">
            <table id="dg" class="easyui-datagrid" style="width:auto;height:auto;border:1px solid #ccc;">
                ...
                <tbody>
                    <?php foreach ($group_mes as $item){ ?>
                        <tr>
                            <td><?= $item['id']; ?></td>
                            <td><?= $item['group_name']; ?></td>
                            <td><?= $item['usr_count']; ?></td>
                            <td><a href="admin_group_manage?edit=<?= $item['id']; ?>">编辑</a> | <a href="admin_group_manage?del=<?= $item['id']; ?>" onclick="return confirm('确认删除?');">删除</a></td>
                        </tr>
                    <?php } ?>
                </tbody>
            </table>
            ...
            <form id="ff" class="easyui-form" method="post" action="" data-options="novalidate:true">
                <div class="easyui-panel" style="padding:5px;height:auto;width:auto;">
                    ...
                    <div class="easyui-panel" title="设置权限" style="width:auto;height:300px;">
                        <ul id="tt" class="easyui-tree" data-options="url:'/admin_manage/ajax_pw_tree<?php echo isset($edit_data) ? "?edit=" . $edit_data['id'] : '' ?>',method:'get',animate:true,checkbox:true,cascadeCheck:false"></ul>
                    </div>
                </div>
            </form>
        </div>
