...
class Md_redis {
    ...
    private $host = '52.76.42.108';
    private $port = 6401;
    private $repeat_reconnected = false;

    public function __construct($host = '', $port = '') {
        $redis_config = _gc('md_redis', 'config');
        if (!empty($host) && !empty($port)) {
            $this->host = $host;
            $this->port = $port;
        } else {
            $this->host = $redis_config['ip'];
    ...
    protected function reportError($msg) {
        ...
        //echo $msg, E_USER_WARNING;
        error_log(date("Y-m-d H:i:s") . "\t" . $msg . "\t" . var_export($this->host, true) . "\n", 3, "d:redis.log");
    }
    ...
    public function PUnsubscribe($pattern = null) {
        if (!empty($pattern)) {
            ...
            return $this->_send(array('PUnsubscribe'));
    }
    ...
    public function SortLimit($key, $offset, $limit, $sort_rule = 'asc') {
        return $this->_send(array('Sort', $key, $sort_rule, 'limit', $offset, ($offset + $limit)));
    }
    ...
    public function zRange($key, $start, $stop, $withscores = false) {
        if ($withscores)
            return $this->_send(array('zRange', $key, $start, $stop, self::WITHSCORES));
        else
            return $this->_send(array('zRange', $key, $start, $stop));
    }
    ...
    public function zRevRange($key, $start, $stop, $withscores = false) {
        if ($withscores)
            return $this->_send(array('zRevRange', $key, $start, $stop, self::WITHSCORES));
        else
            return $this->_send(array('zRevRange', $key, $start, $stop));
    }
